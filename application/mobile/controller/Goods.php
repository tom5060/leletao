...
class Goods extends MobileBase {
     ...
     * 品牌团
     */
    public function brand_team(){
        $brand_id = I('brand_id');

        $brand_name = M('brand')->where('id', $brand_id)->find();

        $lists = M('goods')->where('brand_id', $brand_id)->select();
        $this->assign('lists',$lists);
        $this->assign('brand_id',$brand_id);
        $this->assign('brand_name',$brand_name['name']);
        return $this->fetch();
    }
    ...
    public function goodsInfo(){
        ...
        if($goods['prom_type'] == 1)
        {
            ...
            $goods['market_price'] = $goods['market_price'] ? $goods['market_price'] : 1;// 仿制除数为0的情况
...
//        $collect = M('goods_collect')->where("goods_id=$goods_id and user_id=$user_id")->count();
        ...
        $goods['tuan_price'] = 0;
        if(I('get.tuan/d',0)) {
            $goods['tuan_price'] = D('pintuan_sale')->where(['goods_id' => $goods_id])->getField('price');
        }
        //抢购价格
        $goods['buying_price'] = 0;
        if(I('get.buying/d',0)) {
            $goods['buying_price'] = D('buying_sale')->where(['goods_id' => $goods_id])->getField('price');
        }
        //秒杀价格
        $goods['seckill_price'] = 0;
        if(I('get.seckill/d',0)) {
            $goods['seckill_price'] = D('seckill_sale')->where(['goods_id' => $goods_id])->getField('price');
        }
    ...
    public function search(){
        ...
        $brand_id  && ($filter_param['brand_id'] = $brand_id); //加入帅选条件中                
        $price  && ($filter_param['price'] = $price); //加入帅选条件中
        // $gq = iconv("GBK", "UTF-8", urldecode(trim(I('get.q','')))); // 关键字搜索
        // if($gq==""){
        ...
        if(count($_POST)){
            ...
            $q = iconv("GBK", "UTF-8", urldecode(trim(I('get.q','')))); // 关键字搜索
        }
        // p($q);
        // p("post:");
        // p($_POST);
