...
class User extends MobileBase
{
    ...
    */
    ...
    public function prize(){
        $sta = M('prize_status')->where('id',1)->find();
        if ($sta['status'] == 0) {
            $this->error('抱歉，抽奖尚未开启');
        }else{
            $obj = M("prize")->order("sort")->select();
            $data["dt"]=$obj;
            $mf = $this->MFCount($this->user["user_id"]);//每天免费抽奖次数
            $d = json_encode($data);
            $this->assign('d', $d);
            $this->assign('mf', $mf);
            $this->assign('obj',$obj);
            $this->assign('zcy',$this->LSCount() + 68956); // 历史抽奖次数
            $this->assign('NowDay',$this->LSCount(true) + 3439); // 今日抽奖次数
            $this->assign('zjjl',$this->WinningRecord());
            return $this->fetch();
        }
    }
    public function GPrize(){
        //dump($this->user);
        if($_SERVER['REQUEST_METHOD'] != "POST")
             return;
        //用户已使用免费次数
        $mf = $this->MFCount($this->user["user_id"]);
        if($mf>=5){
            $consume=10;
            if($this->user["pay_points"]<$consume){
                return -1;
            }else{
                M("users")->where("user_id=".$this->user["user_id"])->setDec('pay_points',$consume);
                return $this->StartPrize($this->user["user_id"],$consume);
            }
        }else{
            return $this->StartPrize($this->user["user_id"],0);
        }
    }
    /**
    * 最新中奖记录
    * @param type $nub 最新100条
    * @return type
    */
    public function WinningRecord($nub=100){
        $obj = DB::query("select p.`name`,p.dtime,p.bz,u.nickname from __PREFIX__prize_draw p,__PREFIX__users u  where p.uid=u.user_id and p.`name` !='无奖区' order by dtime desc limit $nub");
        return $obj;
    }
    /**
    * 历史参与统计
    * @param type $isNow false 历史 true 今天
    * @return type
    */
    public function LSCount($isNow=false){
        if($isNow){
            $d["dtime"]= array('egt',date("Y-m-d"));
        }else{
            $d=null;
        }
        $c = M("prize_draw")->field('id,uid')->where($d)->group('uid')->count("id");
        return $c;
    }
    /**
    * 用户某天免费抽奖次数
    * @param type $id 用户id
    * @param type $datetime 日期
    * @return type
    */
    public function MFCount($id,$datetime=null){
        if(!isset($datetime))
            $datetime=date("Y-m-d");
        $c = M("prize_draw")->where(array("uid"=>$id,"consume"=>0,"dtime"=>array('egt',$datetime)))->count("id");
        return $c;
    }
    /**
        * 获取奖品信息
    ...
    */
    public function GetPrize($id){
        $obj = M("prize")->where(array("id"=>$id))->find();
        return $obj;
    }
    ...
    /**
        * 抽奖
        * @param type $id 用户id
        * @param type $consume 消费积分
        * @return type
    */
    public function StartPrize($id,$consume){
        $obj = M("prize")->order("sort")->select();
        $arrs=[];
        foreach ($obj as $k => $v){
            if($v["numb"] != 0){
        ...
        if(count($arrs)){
            $jid = $this->get_rand($arrs);
            $jdata=$this->GetPrize($jid);
            $data = array(
                "uid"=>$id,
                "consume"=>$consume,
                "name"=>$jdata["name"],
            ...
            if ($data['bz'] == '继续加油') {
                $data['status'] = 1;
                $data['desc'] = '已兑奖(奖项：无奖区)(奖品：继续加油)';
                M("prize_draw")->add($data);
            }elseif($data['bz'] == '5个积分'){
                $data['status'] = 1;
                $data['desc'] = '已兑奖(奖项：鼓励奖)(奖品：5个积分)';
                M("prize_draw")->add($data);
                accountLog($data['uid'], 0 , 5 , '抽奖兑换(奖项：鼓励奖)(奖品：5个积分)'); // 记录日志流水
            }elseif($data['bz'] == '10个积分'){
                $data['status'] = 1;
                $data['desc'] = '已兑奖(奖项：鼓励奖)(奖品：10个积分)';
                M("prize_draw")->add($data);
                accountLog($data['uid'], 0 , 10 , '抽奖兑换(奖项：鼓励奖)(奖品：10个积分)'); // 记录日志流水
            }elseif($data['bz'] == '5元优惠卷'){
                ...
                if($coupon['createnum']>0){
                    ...
                    if($remain<=0){
                        $data['status'] = 0;
                        $data['desc'] = $coupon['name'].'已经发放完了';
                        M("prize_draw")->add($data);
                    }
                }

                $data['status'] = 1;
                $data['desc'] = '已兑奖(奖项：三等奖)(奖品：5元优惠卷)';
                M("prize_draw")->add($data);

                $time = time();
                $insert[] = ['cid' => $cid, 'type' => 5, 'uid' => $data['uid'], 'send_time' => $time];
                DB::name('coupon_list')->insertAll($insert);
                M('coupon')->where("id",$cid)->setInc('send_num',1);

            }elseif($data['bz'] == '10元优惠卷'){
                ...
                $data['status'] = 1;
                $data['desc'] = '已兑奖(奖项：三等奖)(奖品：10元优惠卷)';
                M("prize_draw")->add($data);
                ...
                $data['status'] = 1;
                $data['desc'] = '已兑奖(奖项：三等奖)(奖品：30元优惠卷)';
                M("prize_draw")->add($data);
            ...
            if($jdata["numb"] > 0){
                M("prize")->where('id='.$jid)->setDec('numb'); // 数量-1
            }
            return $jid;
        }
        return 0;
...
//        $goods = M('goods')->where("goods_id", $goods_id)->find();
    ...
    public function send_online(){
        ...
        if (IS_POST) {
            if (!empty($user_id)) {
                ...
                $time = time();
                $insert[] = ['cid' => $cid, 'type' => 5, 'uid' => $user_id, 'send_time' => $time];
                ...
                $map['uid'] = $user_id;
                $coupon = M('coupon')->where("id",$cid)->find();
                $coupon_list = M('coupon_list')->where($map)->find();

                if (!$coupon) {
                    $this->error('抱歉，没有此优惠券',U('Index/index'));
                }
    ...
    public function order_sy(){
        $sql="SELECT d.orderid,d.spec,d.`name`,d.address,d.kuaidi,d.zt,f.title,g.original_img FROM `lt_freetrial_dd` d , `lt_freetrial` f,`lt_goods` g WHERE d.fid = f.id and g.goods_id=d.goodid and d.uid=".$this->user_id.";";
        $data = Db::query($sql);
        $data[0]["spec"]=str_replace("|","  ",$data[0]["spec"]);
        $data[0]["address"]=str_replace("|"," - ",$data[0]["address"]);
