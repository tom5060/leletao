...
//        $urlObj["scope"] = "snsapi_base";
    ...
    /**
     * 如果拼团商品已经过期则把商品标注为过期状态
     */
    private function tuanGoodsExpire()
    {
        $map = [
            'end_time' => ['lt', time()]
        ];
        $count = Db::name('pintuan_infos')->where($map)->count();
        if ($count > 0)
        {
            Db::name('pintuan_infos')->where($map)->update(['status' => 2]);
        }
    }
