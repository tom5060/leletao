...
class Order extends Base {
    ...
    */
    ...
    public function detail($order_id){
        ...
        $tuan_status = 0;
        $tuan_status = D('pintuan_infos')->where(['m_orderid' => $order_id,'status' => 1])->getField('status');
        if(empty($tuan_status))
        {
            $tuan_status = D('pintuan_infos')->where(['s_orderid' => $order_id,'status' => 1])->getField('status');
        }

        $this->assign('tuan_status',$tuan_status);
   ...
   {                
    ...
    public function export_order()
    {
        ...
        $strTable .= '<td style="text-align:center;font-size:12px;" width="*">是否拼团</td>';
	    ...
	    if(is_array($orderList)){
	    	...
	    	foreach($orderList as $k=>$val){
	    		...
	    		unset($orderGoods);
                $tuan = $val['is_tuan'] > 0 ? '拼团' : '否';
                $strTable .= '<td style="text-align:left;font-size:12px;">'.$tuan.' </td>';
