...
class User extends Base {
    ...
    public function search_user()
    {
        ...
        // 根据会员id号搜索
        $list = M('users')->where("user_id",$search_key)->find();        
        echo "<input type='hidden' name='user_id' value='{$list['user_id']}' />会员id：{$list['user_id']}&nbsp;&nbsp;&nbsp;&nbsp;昵称：{$list['nickname']}";

        // 根据会员id号查询收货地址

        $dizhi = M('user_address')->where("user_id", $list['user_id'])->order('is_default desc')->select();

        foreach ($dizhi as $key => $value) {
            $province = M('region')->where(array('id'=>$value['province']))->find();
            $city =  M('region')->where(array('id'=>$value['city']))->find();
            $area =  M('region')->where(array('id'=>$value['district']))->find();  

            $duoDz[] = '('.$value['is_default'].')'.'--'.$province['name'].'--'.$city['name'].'--'.$area['name'].'--'.$value['address'].'--'.$value['consignee'].'(收货人)'.'--'.$value['mobile'].'<br />';
        }

        foreach ($duoDz as $k => $v) {
            echo "<p style='color:#000;line-height:25px;'>{$v}</p>";
        }
        exit;
    }
