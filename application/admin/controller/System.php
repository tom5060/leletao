...
//                exit;
     ...
     * 大转盘开关
     */
     public function zhuanpan_status(){
        $data = array(
                'status' => I('status'),
                );
        $status = M('prize_status')->where('id',1)->save($data);
        if ($data) {
            $this->success('修改成功',U('Admin/System/zhuanpan_setting'));
        }else{
            $this->error('修改失败');
        }
     }
      ...
      * 大转盘设置
      */
     public function zhuanpan_setting(){
        $status = M('prize_status')->where('id',1)->find();
        $this->assign('status', $status);

        $zhuanpan_data = M('prize')->order('sort desc')->select();
        $this->assign('zhuanpan_data', $zhuanpan_data);

        $chance_sum = M('prize')->sum('chance');
        $this->assign('chance_sum', $chance_sum);
     ...
     public function zhuanpan_edit(){
        ...
        if (!$_POST) {
            $edit_data = M('prize')->where('id', I('id'))->find();
            ...
            $edit = M('prize')->where('id', I('id'))->save($data);
            if ($edit) {
                $this->success('修改成功',U('Admin/System/zhuanpan_setting'));
            }else{
                $this->error('修改失败');
     ...
     public function winning_list(){
        ...
        if ($status == 2) {
            $winning_list = M('prize_draw')->where('status', 0)->order('id desc')->paginate(15);
        }else{
            $winning_list = M('prize_draw')->order('id desc')->paginate(15);
        }

        // $winning_list = M('prize_draw')->order('id desc')->paginate(15);

        $page = $winning_list->render();

        $data = $winning_list->all();
      ...
      * 自动兑奖
      */
     // public function auto_winning(){
     //    $map['status'] = 0;
     //    $map['bz'] = '30元优惠卷';

     //    $auto_winning = M('prize_draw')->where($map)->order('id desc')->select();

     //    foreach ($auto_winning as $key => $value) {            
     //        Db::name("prize_draw")->where("id",$value['id'])->update(['status'=>1,'desc'=>'auto已兑奖(奖项：三等奖)(奖品：30元优惠卷)']);
     ...
     //        $time = time();
     //        $insert[] = ['cid' => $cid, 'type' => 5, 'uid' => $value['uid'], 'send_time' => $time];
     ...
     public function winning_edit(){
        if (!$_POST) {
            $edit_data = M('prize_draw')->where('id', I('id'))->find();
            $this->assign('edit_data', $edit_data);
            return $this->fetch();
        }else{
            $data = array(
            ...
            $edit = M('prize_draw')->where('id', I('id'))->save($data);
            if ($edit) {
                $this->success('修改成功',U('Admin/System/winning_list'));
            }else{
                $this->error('修改失败');
            }
        }
