...
class Promotion extends Base
{
    ...
    public function flash_sale_set_banner()
    {
        $id     = I('id');
        $status = I('status');
        $ret    = D('flash_sale')->where("id={$id}")->update(['banner' => $status]);
        if ($ret)
        {
            $response = [
                'code' => 200,
    ...
    public function pre_sell_info()
    {
        ...
        $id                 = I('id');
        $info['start_time'] = date('Y-m-d H:i:s');
        $info['end_time']   = date('Y-m-d 23:59:59', time() + 3600 * 24 * 7);
        if ($id > 0)
        {
    ...
    public function pre_sell_del()
    {
        ...
        if ($id)
        {
            ...
            else
            {
                ...
                M('goods')->where(array(
                        'prom_type' => 4,
                        'goods_id'  => $goods_activity['goods_is']
                    )
                )->save(array('prom_id' => 0, 'prom_type' => 0, 'is_on_sale' => 0));
            }
    ...
    public function pre_sell_success()
    {
        ...
        if (count($pre_sell_order_id_list) > 0)
        {
            ...
            M('order_goods')->where(array(
                    'order_id' => array(
                        'IN',
                        $pre_sell_order_id_list
                    )
                )
            )->save(array('member_goods_price' => $pre_sell_final_price));
            //获取所有更新后的订单商品的商品总价
            $pre_sell_order_goods = M('order_goods')->field('order_id,SUM(goods_num*member_goods_price) as goods_amount')->where(array(
                    'order_id' => array(
                        'IN',
                        $pre_sell_order_id_list
                    )
            ...
            foreach ($pre_sell_order_goods as $key => $val)
            {
                ...
                if ($pre_sell_order['paid_money'] > 0 && $pre_sell_order['pay_status'] == 2)
                {
                    $save_data = array(
                        ...
                        'order_amount' => $pre_sell_order_goods[$key]['goods_amount'] - $pre_sell_order['paid_money']
                        //需要支付的尾款
                    );
        ...
        M('goods')->where(array(
                'prom_type' => 4,
                'goods_id'  => $goods_activity['goods_id']
            )
        )->save(array('prom_id' => 0, 'prom_type' => 0, 'is_on_sale' => 0));
