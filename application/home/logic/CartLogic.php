...
class CartLogic extends Model
{
    ...
    {                   
        ...
        {
             $where .= " and user_id = $user[user_id] and goods_num != 0";
             // 给用户计算会员价 登录前后不一样             
        }           
        else
        {
    ...
    function cart_freight2($shipping_code, $province, $city, $district, $weight)
    {
        ...
        if ($shipping_area_id == false) {
            ...
            // 如果是偏远地区，则is_default = 0 
            if($province == 46047 || $province == 41103 || $province == 45753 || $province == 4670 || $province == 47493 || $province == 47494 || $province == 47495){
                $faraway = 0;
            }else{
                $faraway = 1;
            }
            ...
            $shipping_area_id = M("ShippingArea")->where(['shipping_code'=>$shipping_code,'is_default'=>$faraway])->getField('shipping_area_id');
        }
        if ($shipping_area_id == false)
            return 0;
     ...
     * @param int $is_tuan 是否是拼团订单
     * @return type $order_id 返回新增的订单id
     */
    public function addTuanOrder($goods_id,$tuan_price,$goods_num,$user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price,$user_note='')
    {
        ...
        $goods = M('goods')->where("goods_id", $goods_id)->cache(true,TPSHOP_CACHE_TIME)->find();
        ...
        $data2['goods_price']        = $tuan_price; // 拼团价
        $data2['spec_key']           = ''; // 商品规格
        $data2['spec_key_name']      = ''; // 商品规格名称
        $data2['member_goods_price'] = $tuan_price; // 会员折扣价
        $data2['cost_price']         = $goods['cost_price']; // 成本价
        $data2['give_integral']      = $goods['give_integral']; // 购买商品赠送积分
        $data2['prom_type']          = 4; // 0 普通订单,1 限时抢购, 2 团购 , 3 促销优惠 4 拼团
